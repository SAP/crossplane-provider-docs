...
- Bottleneck is typically I/O wait, not CPU - concurrency can safely exceed CPU cores
...
**What it controls:** A token-bucket rate limiter that throttles ALL reconcile requests from this provider.
...
- Sequential backoff: first failure → 1s, then 2s, 4s, 8s, ..., up to 60s max

**Key interaction:** `GlobalRateLimiter` (tokens/sec) = `MaxConcurrentReconciles` (workers). Setting RPS > concurrency wastes tokens; setting RPS < concurrency starves workers.
...
- Polls consume tokens from GlobalRateLimiter
- After RequeueAfter, backoff is cleared and next poll begins immediately
...
**Tuning:** If you increase MaxConcurrentReconciles, QPS/Burst scale automatically (N × 5). Do not set these manually unless you understand the call profile of your provider.
...
**Consideration:** For single-replica deployments, leader election is pure overhead. Some providers allow disabling it.
