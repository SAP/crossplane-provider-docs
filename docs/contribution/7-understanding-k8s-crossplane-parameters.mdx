...
- **[Contribution Possibilities](#contribution-possibilities)** — change defaults or expose new flags in provider code
...
Everything else — timeouts, backoff, jitter, leader election — is fixed at the values set by the provider developer. If defaults don't work for your scale, see [Contribution Possibilities](#contribution-possibilities).
...
- A token-bucket rate limiter (RPS = N) gates every reconcile — if no token is available, the item is requeued
...
**What it controls:** How often external state is checked for drift.
...
> **Jitter note:** BTP and CF providers have no built-in jitter. Without jitter, all resources requeue simultaneously after a restart. For large landscapes (500+), consider raising the poll interval to spread the load naturally, or open an issue to request jitter support.
...
| **Add poll jitter** | Implement `PollJitterPercent` in the controller setup | Critical for BTP and CF on large landscapes |
...
| **Disable/tune leader election** | Pass `LeaderElection: false` or adjust lease durations | Reduces KubeAPI overhead for single-replica deployments |
| **Adjust QPS/Burst multipliers** | Change the N×5 / N×10 formula in client config | Useful if a provider's reconcile call profile differs significantly |
...
2. **No poll jitter on large landscapes** — All resources requeue simultaneously after restart; raise poll interval to compensate
