...
If you just need a practical starting point, use this table first and then read the detailed sections below for the reasoning and trade-offs behind it.

| Landscape size | Suggested `--max-reconcile-rate` | Suggested poll interval | Notes |
|----------------|----------------------------------|--------------------------|-------|
...
:::tip BTP provider configuration
For BTP-specific instructions on configuring `--max-reconcile-rate`, `--poll`, `--sync`, backoff, and leader-election flags via `DeploymentRuntimeConfig`, see [Configure Controller Flags][btp-controller-flags].
:::
...
flowchart TD
    ...
    maxconc["MaxConcurrentReconciles = N<br/>(per controller)"]
...
**What it controls:** The number of worker goroutines **per controller** that can reconcile resources of that controller's type in parallel. It is not a single provider-wide concurrency cap.

**Default:** 1 (framework default)

**Configured via:** --max-reconcile-rate (the same value is applied as `MaxConcurrentReconciles` per controller and is coupled with other parameters)

**How it works:**
- Each controller spawns exactly N workers (fixed count, not elastic). If a provider registers multiple controllers, each controller gets its own worker pool, so the theoretical provider-wide concurrency from this setting alone is roughly N × number of active controllers.
...
## Detailed Tuning by Landscape Size
...
[btp-controller-flags]: /docs/crossplane-provider-btp/docs/end-user-guides/setup/configure-controller-flags
